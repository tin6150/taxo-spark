...
## #!/bin/env python
##    /prog/python/2.7.9-goolf-1.5.14-NX/bin/python   (2016.0304 converting back to this for v.6) 
## #!/prog/python/3.4.2-goolf-1.5.14-NX/bin/python3   (code now requires python3).
...
def getParentByTaxidRank( taxid, rank, tH ) :
        rankOfInputTaxid = tH.getRankByTaxid( taxid ) 
        ...
        if r.lower() == rank.lower() :        # found what was asked for
                ...
                return getParentByTaxidRank( t, rank, tH )

## ++ hmmm, this method isn't used anywhere in this script.  and don't seems anything else calling it.
##def getRankByGi( gi ) : 
def getRankByAccVer( accV, tH ) : 
    ##taxid = getTaxidByGi( gi )
    taxid = tH.getTaxidByAccVer( accV )
    return tH.getRankByTaxid( taxid )


## ++ hmm... this method don't seems to be used either.
...
def getRankNameByAccVer(gi,rank, tH):
    ##taxid = getTaxidByGi( gi )
    taxid = tH.getTaxidByAccVer( gi )
    #dbg( "taxid=>%s" % taxid )
    #return getLineageByTaxid( taxid, rank )
    return getRankNameByTaxid( taxid, rank, tH )
...
def getRankNameByTaxid( taxid, rank, tH ):
    currentRank = tH.getRankByTaxid( taxid )
    name = tH.getNameByTaxid( taxid )
    ...
    parentTaxid = tH.getParentByTaxid( taxid )
...
getLineageByTaxid = getRankNameByTaxid  # def alias
...
def getTraceByTaxid( taxid, tH ):
    tree = [ ]
    return getTraceByTaxid2( taxid, tree, tH )

##def getTraceByGi( gi ):
def getTraceByAccVer( gi, tH ):
    tree = [ ]
    return getTraceByTaxid2( tH.getTaxidByAccVer( gi ), tree, tH )
...
def getTraceByTaxid2( taxid, tree, tH ):
    currentRank = tH.getRankByTaxid( taxid )
    name = tH.getNameByTaxid( taxid )
    ...
    if( parentTaxid == taxid ) :
        ...
        return getTraceByTaxid2( parentTaxid, tree, tH )
...
def getTraceByTaxid2_old( taxid, tree, tH ):
    currentRank = tH.getRankByTaxid( taxid )
    name = tH.getNameByTaxid( taxid )
...
def file2accVerList( filename, colidx, ifs, tH ):
    # lots of things need to be changed to use tH ++ TODO
    # tH is for taxoHandle, an object with sparkSQL context for taxonomy db queries
    ...
    for line in f:
        ...
        if( g == "" ) :
            rejectRowCount += 1
            dbg( 4, "empty accession.v, breaking away.  ***L269" )
            ##break    # this will terminate all for loop and stop processing the file!!  don't use!!
            continue #pass
        if g not in accVerList :
                taxid = tH.getTaxidByAccVer( g )               # formerly taxid = getTaxidByGi( g )
                rank = tH.getRankByTaxid( taxid )              # "species" or "no_rank" in subspecies ??
...
def summarizeAccVerList( giList, tH ):
        ...
        for g in giList :
                ...
                tmpParentTaxid = tH.getParentByTaxid( giTaxid )                       # during this first pass, likely no-rank, which is sub-species
                ...
                if giTaxid not in resultTable4[currentRankName] :
                        parentTaxid = getParentByTaxidRank( giTaxid, nextUpRank.name, tH )  # eg looking for "species" taxid
                        rankName = tH.getNameByTaxid(   giTaxid )
        ...
        for rank_item in workingRankSet :                       # this need to be in order, going up taxo tree.
                ...
                for childTid in resultTable4[childRank.name] :
                        ...
                        if tid not in resultTable4[currentRank.name] :
                                ...
                                parentTaxid = getParentByTaxidRank( tid, parentRank.name, tH )    
                                dbg( 4, "for tid: %8s got parentTaxid %s" % (tid, parentTaxid) )
                                rankName = getLineageByTaxid(   tid, currentRank.name, tH )
        ...
        for childTid in resultTable4[belowTopRank.name] :
                ...
                if tid not in resultTable4[topRank.name] :
                        ...
                        rankName = getLineageByTaxid( tid, topRank.name, tH )
...
def prettyPrint( table, outfile, tH ) :
